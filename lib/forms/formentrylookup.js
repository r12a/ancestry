function listRelations (individual) { 
	...
	if (db[individual].fg) { 
		for (i=0;i<db[individual].fg.length;i++) {
			if (db[db[individual].fg[i][1]]) lines.push('<p>'+db[individual].fg[i][1]+'</p>')
			}
		}
	...
	if (person.father || person.mother) {
		if (person.father && db[person.father]) lines.push('<p>'+person.father+'</p>')
		if (person.mother && db[person.mother]) lines.push('<p>'+person.mother+'</p>')
		}
	...
	if (db[person.father]) { 
			if (a = db[person.father].father) if (db[a]) lines.push('<p>'+a+'</p>')
			if (a = db[person.father].mother) if (db[a])  lines.push('<p>'+a+'</p>')
		if (db[person.mother]) {
			if (a = db[person.mother].father) if (db[a]) lines.push('<p>'+a+'</p>')
			if (a = db[person.mother].mother) if (db[a]) lines.push('<p>'+a+'</p>')
			}
		}
	...
	if (db[individual].fg) { 
		for (i=0;i<db[individual].fg.length;i++) {
			for (j=2;j<db[individual].fg[i].length;j++) if (c = db[individual].fg[i][j]) if (db[c]) lines.push('<p>'+c+'</p>')
			}
		}
				...
				if (db[person.fg[i][j]]) {
					...
					if (db[child].fg) {
						for (var k=0;k<db[child].fg.length;k++) {
							...
							if (cspouse && db[cspouse]) lines.push('<p>'+cspouse+'</p>')
							}
						}
					}
				}
