...
function summarise (node) {
	...
	for (var i=0;i<records.length;i++) {
		if (records[i].className === 'recordDate') out += '<br/>\t</p><p>'
		if (records[i].className === 'keypoints' && records[i].textContent !== '') {
			out += '<br/>'
		...
		else if (records[i].className === 'recordTitle') out += '<br/>'+upperCaseFirst(records[i].textContent)+' '
		else if (records[i].className === 'discn') out += '<br/>'+upperCaseFirst(records[i].textContent)+' '
...
function getDBParents (phrase, person) { console.log(person)
	var out = ''
...
function redisplay (everything, id, summary) { 
	...
	out += '<a id="gotoTree" href="tree.html?person='+id+'">Tree</a>'
	...
	for (i=1;i<records.length;i++) {
		...
		for (f=1;f<fields.length;f++) {
			...
			switch (parts[0]) {
				 ...
				 case 'gps': //console.log(usegps, who, what)
				 		if (usegps) {
				 		temp=''
		...
		*/
		...
		else out += '><span>'+getAge('', info.date, year, bdate, born, '')+'</span><span style="font-size:1px;line-height:1px;"> &bull; </span><span>'+year+'<span>'
...
function checkBMD (individual) {
	...
	for (i=0; i<lines.length; i++) {
		if (! currentIncludes.match(lines[i])) missingIncludes.push('<script src="'+lines[i]+'.js" type="application/javascript"></script>')
		}
	...
	// report
	if (missingIncludes.length > 0 || errors.length > 0) {
	...
	if (missingIncludes.length > 0) {
		console.log('ADD THESE INCLUDES:')
		for (i=0;i<missingIncludes.length;i++) {
			console.log(missingIncludes[i])
			p = document.createElement('div')
			p.textContent = missingIncludes[i]
			bottomOfPage.appendChild(p)
			}
		}
