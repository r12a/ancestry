		...
		for (var i=0;i<parent.fg.length;i++) {
			for (var j=2;j<parent.fg[i].length;j++) {
				if (db[parent.fg[i][j]]) {
					...
					if (data[sibling] && data[sibling].death && data[sibling].death.year) dyear = data[sibling].death.year
					else dyear = db[sibling].d
					}
				if (byear >= birth && byear <= death && sibling !== individual) records.push('§ '+byear+' '+siblingType+' born '+sibling)
				if (dyear >= birth && dyear <= death && sibling !== individual) records.push('§ '+dyear+' '+siblingType+' dies '+sibling)
				}
			}
		}
