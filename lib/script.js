			...
			switch (parts[0]) {
				 case 'birth':  info.id = parts[1].trim()
								...
								if (otherPerson) {
									...
									if (db[parts[1].trim()].bdate) fields.push('date§'+db[parts[1].trim()].bdate)
...
function checkBMD (individual) {
	...
	for (i=0; i<lines.length; i++) {
		if (! currentIncludes.match(lines[i])) missingIncludes.push('<script src="'+lines[i]+'" type="application/javascript"></script>')
		}
