...
function summarise (node, thisPerson) {
	document.getElementById('summaryWin').style.display = 'block'
	var out = '<button class="summaryClose" onclick="this.parentNode.style.display=\'none\'">X</button>'
    ...
    out += '<h4>Places lived</h4><ul>'
    var placeArray = [...placeSet]
    for (var i=0;i<placeArray.length;i++) if (placeArray[i] !== '') out += `<li>${ placeArray[i].trim() }`
    out += '</ul>'
	//if (document.getElementById('places')) out += document.getElementById('places').outerHTML
    ...
    var occArray = [...occSet]
    for (var i=0;i<occArray.length;i++) if (occArray[i] !== '' && occArray[i] !== 'Child') out += `<li>${ occArray[i].trim() }`
    out += '</ul>'
	//out += document.getElementById('places').outerHTML
    
    var witness = showWitnesses(thisPerson)
    if (witness !== '<h4>Named as witness:</h4><ul></ul>') out += witness
	
    var present = showAppearsIn(thisPerson)
    if (present !== '<h4>Appears in record:</h4><ul></ul>') out += present
...
function addFIcon (text, type) {
	...
	if (text) {
		if (type === 'gps') {
			...
			out = `<p class="floatedIconText"><a target="_blank" class="gpslink" href="http${ parts[1] }"><img src="lib/i/map.png" alt="${ parts[0] }" title="${ parts[0] }"> ${ parts[0] }</a></p>`
			return out.replace(/§/,':')
			}
		if (type === 'record') {
			if (text.match(/url:/)) {
...
function redisplay (researchNotes, id, summary ) { 
	...
	out += '<span id="summarise" onclick="summarise(this, thisPerson)">Lists</span>'
	...
	if (person.father) {
        ...
        out += `<span ${ rootConnected }>${ getName('', person.father, 'both', true) }</span>`
        }
	if (person.mother && person.father ) out += ' \u2022 '
	if (person.mother) {
        if (markList.has(person.mother)) rootConnected = ` class="rootConnected"`
        else rootConnected = ''
        out += `<span ${ rootConnected }>${ getName('', person.mother, 'both', true) }</span>`
        }
	...
	if (sid.length > 0) {
		for (c=0;c<sid.length;c++) {
			...
			out += `<span ${ rootConnected }>${ getName('', sid[c], 'both', true) }</span>`
	...
	if (cid.length > 0) {
		for (c=0;c<cid.length;c++) {
			if (cid[c] === 'a_child') {
				...
				temp.push(`<span ${ rootConnected }>${ getName('', cid[c], 'given', true) }</span>`)
				}
			}
		}
        ...
        if (person.fg) {
            for (i=0;i<person.fg.length;i++) {
                ...
                if ((person.fg[i][0] === '' || person.fg[i][1] === '?') && person.fg[i].length > 2) {
                    ...
                    else intro += ` but we don't yet know of any children`
                    intro += '. '
                    }
                }
            }
...
// THESE MAY NEED TO BE CHANGED TO birthTS and deathTS respectively, if actually used
    ...
    for (var e=0;e < events.length; e++) {
        ...
        if (event.person !== 'event' && typeof db[event.person] === 'undefined' && ! event.person.includes('^') && ! event.relation.includes('spouse')) { console.log(exit,'redisplay: Person undefined.', event.relation); continue }
        ...
        if (event.type === 'dies' && event.relation === 'self') {
            ...
            if (event.of) record += ' '+person.pron+' lived at '+event.of
            if (event.of && event.place && event.place !== event.of) record += ', and '+person.pron.toLowerCase()+' died at '+event.place
            else if (event.place) record += ' '+person.pron+' died at '+event.place
            record += '.'
        ...
        // relative dies
            ...
            //if (event.relation === 'husband') {
                if (event.of && event.place) record += ' '+getName('', event.person, 'kx', true)+' was living at '+event.of+', and died at '+event.place+'.'
                else if (event.of) record += ' '+getName('', event.person, 'kx', true)+' was living at '+event.of+'.'
...
console.log("PLACES"+person.places)
    ...
    /*if (person.places) {
        ...
        for (p=0;p<placeRecords.length;p++) {
            if (typeof placesDB[placeRecords[p]] === 'undefined') {
                console.log('Couldnt find:'+placeRecords[p])
                continue
                }
            if (placesDB[placeRecords[p]].gps || placesDB[placeRecords[p]].desc) {
                //console.log("Going forward with:"+placeRecords[p])
                 placesOut += `<span class="placesRecord">${ placesDB[placeRecords[p]].p }`
                 if (placesDB[placeRecords[p]].desc) placesOut += `<a target="_blank" href="${ placesDB[placeRecords[p]].desc }"><img src="lib/i/info.png" alt="Info"></a>`
                 if (placesDB[placeRecords[p]].gps) placesOut += `<a target="_blank" href="${ placesDB[placeRecords[p]].gps }"><img src="lib/i/map.png" alt="GPS"></a>`
                 placesOut += "</span>"
                 }
            }
    ...
    allNames.forEach(name => { 
        if (db[name].marriages) {
            ...
            for (var i=0;i<spouses.length;i++) {
                if (db[name].marriages[spouses[i]].present && db[name].marriages[spouses[i]].present.includes(person)) out += 
                ...
                if (db[name].death && db[name].death.present && db[name].death.present.includes(person)) out += 
                ...
                if (db[name].birth && db[name].birth.present && db[name].birth.present.includes(person)) out += 
...
function showCensusEntries (person) {
    ...
    allNames.forEach(name => { 
        if ((censi[name].head && censi[name].head.includes(person))) {
            out += `<li>${ name.substr(0,4)} head</li>`
            }
        if (censi[name].wife && censi[name].wife.includes(person)) {
            out += `<li>${ name.substr(0,4)} wife</li>`
            }
        if (censi[name].children) {
            for (i=0;i<censi[name].children.length;i++) if (censi[name].children[i].includes(person)) out += `<li>${ name.substr(0,4)} child</li>`
            }
        if (censi[name].others) {
            for (i=0;i<censi[name].others.length;i++) if (censi[name].others[i].includes(person)) out += `<li>${ name.substr(0,4)}other</li>`
            }
        if (censi[name].serv) {
            for (i=0;i<censi[name].serv.length;i++) if (censi[name].serv[i].includes(person)) out += `<li>${ name.substr(0,4)} servant</li>`
            }
        if (censi[name].visitors) {
            for (i=0;i<censi[name].visitors.length;i++) if (censi[name].visitors[i].includes(person)) out += `<li>${ name.substr(0,4)} visitor</li>`
            }
        })
    out += '</ul>'
