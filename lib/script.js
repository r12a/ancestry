...
function getName (phrase, id, part, uselink) {
	...
	startTag = endTag = ''
	if (uselink && person) { 
		if (person.p) startTag = '<a href="person.html?person='+id+'"'
...
function redisplay (everything, id, summary) { 
	...
	out += '<a style="margin-right:3em;" href="index.html">Index</a> '
	out += '</div>'
	out += '<div id="top"></div>'
	...
	if (data[id].intro) summary.innerHTML = data[id].intro
	else summary.innerHTML = getName('', id, 'gkf', false)
