...
function summarise (node, thisPerson) {
    ...
    if (mentioned !== '<h4>Appears in record:</h4><ul></ul>') out += mentioned
...
function redisplay (researchNotes, id, summary ) { 
    ...
    onchange="if (this.value !== '') openWindow(\'lib/forms/'+this.value+'.html?project=${ project }&person=${ thisPerson }'); this.value=''">
        <option value="">&nbsp;</option>
        ...
        <option value="censusentry">Census</option>
        </select></label>`
