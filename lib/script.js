...
function getNameOLD (phrase, id, part, uselink) {
...
function getNameIMPROVED (phrase, id, part, uselink) {
	// part identifies the format of the name: g, given; k, knownas; f,family, mX,married where X indicates which married name
	if (! db[id]) return id
	if (part.match('x')) { part=part.replace('x',''); uselink = false }  // shortcut for in page names
	var person = db[id]
	...
	if (part === 'gfm' || part === 'kfm') {
        if (person.f && person.m) allSurnames = '('+person.f+')'
        else if (person.f) allSurnames = person.f
        else allSurnames = '(?)'
		if (person.m) {
			for (var i=0;i<person.m.length-1;i++) allSurnames += ' ('+person.m[i]+')'
            allSurnames += ' '+person.m[person.m.length-1]
			}
	...
	if (part === 'gf++') {
		...
		return startTag+g+' '+allSurnames+endTag
		}
	if (part === 'gf+++') {
        if (person.f) allSurnames = '('+person.f+')'
        else allSurnames = ' (?)'
		if (person.m) {
			allSurnames += ' ('+person.m[0]+')' + ' ('+person.m[1]+')'
...
function getNameMORE (phrase, id, part, uselink) {
	// part identifies the format of the name: g, given; k, knownas; f,family, mX,married where X indicates which married name
	if (! db[id]) return id
	if (part.match('x')) { part=part.replace('x',''); uselink = false }  // shortcut for in page names
	var person = db[id]
...
function redisplay (everything, id, summary) { 
	...
	var dob = id.split('_')
	out += '<div id="topMenu" style="text-align:start;">'
	...
	out += '<a id="gotoTree" style="float:right; color:chocolate; margin-inline-end: 2rem;" href="tree.html?project='+window.project+'&person='+id+'">🌳 Show in Tree</a>'
	out += '<a style="margin-right:3em; color:chocolate; float: right;" href="index.html?project='+project+'">Show Index</a> '
	out += '</div>'
	out += '<div id="top"></div>'
	...
	// draw top banner
	out += '<div id="banner"><div id="pagetitle">'+getName('', id, 'gfm', false)+'<br><span id="bannerdates">'+person.b+'\u2013'+person.d
	...
	for (i=1;i<records.length;i++) {
		...
		for (f=1;f<fields.length;f++) {
			...
			switch (parts[0]) {
				 ...
				 case 'deathPlace': info.deathplace = parts[1].trim(); break
        ...
        if (researchNotes !== '') {
            ...
            out += ' style="text-align:center; margin-right: 3em; margin-top: 2em; text-transform: uppercase; font-family: \'Source Sans Pro\', \'Helvetica Neue\', Arial, sans-serif; font-size: 100%; color:#666;">Notes &amp; research</summary>\n<p style="white-space: pre-wrap; font-size:90%;margin:1em;">'
            out += researchNotes
            out += '</p>\n</details>'
            }
