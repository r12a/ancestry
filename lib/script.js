...
function redisplay (everything, id, summary) { 
	...
	out += '<a title="Show Descendants." href="descendants.html?project='+project+'&person='+id+'">&#x25b3;</a> '
    
	out += '<a title="Show Ancestors" href="precedents.html?project='+project+'&person='+id+'">&#x25bd;</a> '
	...
	for (i=1;i<records.length;i++) {
		...
		for (f=1;f<fields.length;f++) {
			...
			switch (parts[0]) {
				 ...
				 case 'deathPlace': info.deathplace = `<span class="place">${ parts[1].trim() }</span>`; break
			...
			case 'birth':
					...
					if (info.siblings.length > 0) {
						if (info.siblings.length === 1) record += '<p>'+pron+' had an elder sibling, '+getName('', info.siblings[0].trim(), 'given', true)+'.</p>'
							...
							record += '<p>'+pron+' had '+info.siblings.length+' elder siblings who were still living: '
